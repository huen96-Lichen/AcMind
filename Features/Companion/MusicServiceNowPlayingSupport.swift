//
//  MusicServiceNowPlayingSupport.swift
//  AcMind
//
//  Shared now playing probes and AppleScript helpers used by MusicService.
//
...
enum MusicServiceAppleScript {
    static func run(_ source: String, logger: Logger) async throws -> String {
        try await withCheckedThrowingContinuation { continuation in
            ...
            if let script = NSAppleScript(source: source) {
                ...
                if let error = error {
                    ...
                    logger.debug("AppleScript error code=\(code, privacy: .public) message=\(message, privacy: .public)")
...
enum MediaRemoteNowPlayingProbe {
...
enum PlayerNowPlayingProbe {
    ...
    static func fetch(logger: Logger) async -> NowPlayingSnapshot? {
        for adapter in adapters {
            ...
            do {
                let result = try await MusicServiceAppleScript.run(adapter.script, logger: logger)
...
enum QishuiMusicNowPlayingProbe {
    struct Candidate {
        let text: String
        let frame: CGRect
    }
    ...
    private static func fetchWithAppleScript(logger: Logger) async -> NowPlayingSnapshot? {
        do {
            ...
            return ""
            """
            let result = try await MusicServiceAppleScript.run(script, logger: logger)
    ...
    private static func rectValue(of element: AXUIElement, attribute: CFString) -> CGRect? {
        ...
        let value = unsafeDowncast(rawValue as AnyObject, to: AXValue.self)
...
func isApplicationAvailable(bundleIdentifier: String) -> Bool {
