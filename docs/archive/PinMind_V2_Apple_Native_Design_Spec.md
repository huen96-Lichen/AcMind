...
> **PinMind 应该看起来像一个「Mac 原生应用」，而不是一个「运行在 Mac 上的 Web 应用」。**
...
Apple HIG 明确推荐使用 SF Symbols 作为界面图标。PinMind V2.0 应：
...
PinMind 作为 macOS 应用，必须提供完整的菜单栏：
...
V1.0 规范文档（`PinMind_UI设计规范/` 目录）在 V2.0 落地完成后标记为归档。
