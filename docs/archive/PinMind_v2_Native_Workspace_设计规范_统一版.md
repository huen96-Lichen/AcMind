# PinMind v2.0 Native Workspace 设计规范
...
> **版本说明**：本规范整合自《PinMind v2.0 Native Workspace 视觉设计规范 v2.1》与《PinMind V2.0 Apple 原生风格设计规范 V2》，作为 PinMind v2.0 后续 UI 重构的统一依据。

---

## 0. 文档定位

PinMind v2.0 已经与早期 PinStack / PinMind 原型阶段产生明显版本差距。

早期阶段更像是功能验证、AI 蒸馏流程验证、本地优先存储验证、Obsidian 导出验证、Capture / Inbox / AI Console 等能力堆叠。

而 PinMind v2.0 的目标是成为一个真正可长期使用的本地优先 AI 知识整理工具。

因此，从 Phase 12.3 开始，视觉方案需要从"功能能用"进入"成熟产品体验"。

本规范的目标不是让界面变得炫酷，而是让 PinMind v2.0 变得更像 macOS 原生效率应用、更像 ChatGPT / Codex 一类现代 AI 工作台——更清楚、更简洁、更高效、更统一、更值得长期使用。

---

## 1. 一句话定义

PinMind v2.0 是一个面向 macOS 的本地优先 AI 知识整理工作台。
...
```text
PinMind Native Workspace
```

中文解释：**苹果原生式知识工作台**

PinMind v2.0 不再追求强烈的"AI 科技感"或"未来玻璃感"，而是追求一种更成熟、更克制、更稳定的 macOS 应用质感。
...
> PinMind 应该看起来像一个 Mac 原生应用，而不是一个运行在 Mac 上的 Web 应用。
...
用户打开 PinMind v2.0，第一眼应该知道：我现在有什么内容待处理、下一步应该做什么、哪些内容已经整理完成、哪些任务失败或需要配置、最近导出了什么 Markdown。
...
PinMind v2.0 主导航应保持 Phase 12.2 已收束后的结构：
...
PinMind v2.0 只保留一个品牌强调色。推荐继续保留 PinMind / Acore 的橙色，但必须克制使用。
...
PinMind v2.0 不是强玻璃风格应用。材质用于建立层级，不用于制造炫酷感。
...
PinMind v2.0 作为 macOS 应用，应逐步补齐菜单栏能力。
...
**目标**：配置 PinMind v2.0，而不是展示所有开发调试能力。
...
```text
[ ] PinMind v2.0 的视觉方向明确为 PinMind Native Workspace
...
```text
1.  是否符合 PinMind Native Workspace，而不是 Glass / Cyber / AI OS 风格
...
PinMind v2.0 不是完全推翻 V1.0，而是在 V1.0 的产品逻辑和交互设计基础上，将视觉语言从「自定义暖阳橙」切换为「Apple 原生风格」。
