...
日期：2026-04-30
版本：0.2.2
适用范围：换电脑后继续推进 PinMind V2.1 开发

---

## 1. 项目定位

PinMind 是一个**本地优先的 AI 记忆蒸馏器**（Local-first AI Memory Distiller），基于 Electron + React + TypeScript 技术栈。
...
| Preload API | ✅ 完成 | window.pinmind.capture.record() + getAvailableTypes() |
...
```typescript
window.pinmind.capture.record(input)        // 统一捕获
window.pinmind.capture.getAvailableTypes()  // 获取可用类型
```
...
1. **Markdown 规范来源**：`pinmind_output_spec_pack/` 模板包
