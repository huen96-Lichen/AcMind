...
function isMissingFile(payload: AppErrorPayload): boolean {
  return payload.code === 'FILE_MISSING' || payload.code === 'RECORD_NOT_FOUND' || /ENOENT|does not exist/i.test(payload.detail ?? '');
}
