...
export function useCaptureSize(runtime: RuntimeSettings | null): UseCaptureSizeReturn {
  ...
  const applyRatio = (ratio: CaptureRatioOption) => {
    ...
    const nextHeight = Math.max(1, Math.round((width * (ratio.height ?? 1)) / (ratio.width ?? 1)));
  ...
  const onWidthChange = (nextValue: string) => {
    ...
    const nextHeight = Math.max(1, Math.round((width * (selectedRatio.height ?? 1)) / (selectedRatio.width ?? 1)));
    setCustomHeight(String(nextHeight));
  };
  ...
  const onHeightChange = (nextValue: string) => {
    ...
    const nextWidth = Math.max(1, Math.round((height * (selectedRatio.width ?? 1)) / (selectedRatio.height ?? 1)));
